...
grid_file = '/export/grunchfs/unibjerknes/milicak/bckup/noresm/CORE2/Arctic/DATA/gfdl-mom/grids_bathymetry/ocean.static.nc';
grid_file2 = '/export/grunchfs/unibjerknes/milicak/bckup/noresm/CORE2/Arctic/DATA/gfdl-mom/grids_bathymetry/basin_mask.nc';
lon = ncread(grid_file,'geolon_t');
lat = ncread(grid_file,'geolat_t');
mask = double(ncread(grid_file2,'tmask'));
mask(isnan(mask)==0)=1;
...
lon3 = [-166.1083 -171.4633 -171.6384 -166.1209 -166.1083];
lat3 = [65.7377   66.2030   65.5881   65.0371   65.7377];
...
tlon_varid=netcdf.defVar(ncid,'lon','double',[ni_dimid]);
...
tlat_varid=netcdf.defVar(ncid,'lat','double',[nj_dimid]);
...
slp_varid=netcdf.defVar(ncid,'tr_mask','double',[ni_dimid nj_dimid time_dimid]);
%slp_varid=netcdf.defVar(ncid,'tr_mask','double',[time_dimid ni_dimid nj_dimid]);
...
netcdf.putVar(ncid,tlon_varid,lon(:,1));
netcdf.putVar(ncid,tlat_varid,lat(1,:));
%netcdf.putVar(ncid,slp_varid,in);
% Close netcdf file
netcdf.close(ncid)
ncwrite(ncfile,'tr_mask',in)
