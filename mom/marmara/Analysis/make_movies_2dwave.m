...
if 1
root_folder = '/okyanus/users/milicak/tmp/MOM6-examples/Projects/marmara_gc/';
...
for k=1:60
    ...
    offset = 200;
    data_plot = squeeze(eta(:,:,1,k)).*mask2;
    data_plot(mask2==0)=NaN;

    %m_proj('lambert','lon',[26.8 28.3],'lat',[36.5 37.2]);
    % whole_domain
    m_proj('lambert','lon',[26.5 30.5],'lat',[40.2 41.1]);

    %data_plot(mask2==0)=NaN;
    bb = m_pcolor(lon,lat,land);shading flat    
    hold on
    ...
    m_grid
    printname=['gifs/Marmara_ssh_2d' no];                                           
