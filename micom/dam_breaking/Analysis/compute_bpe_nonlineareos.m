...
dt=450; %(time(2)-time(1))*86400; %seconds output frequency
...
filename=[foldername 'Exp04_1_hm_' num2str(years) '.' no '.nc']
if(exist(filename)~=0)
for n=1:30
...
bottom_press=squeeze(nansum(dp,1));
press(Nz+1,:,:)=bottom_press;
for k=Nz:-1:1
  press(k,:,:)=press(k+1,:,:)-dp(k,:,:);
end
...
q1=q1+(area.*(squeeze(press(1,:,:)).*(squeeze(eosben07_p_alpha(p1./1e4,p0./1e4,th,salt).*1e4))./grav));
q2=q2+(area.*(squeeze(eosben07_p_alphap(p1./1e4,p0./1e4,th,salt)/grav)).*1e4*1e4);
end
dnm=-H.*area.*(squeeze(press(Nz+1,:,:)-press(1,:,:)))+q1-q2;
PE(end+1)=nansum(dnm(:));
...
p1=press_s(k);
p0=press_s(k+1);
q1=q1+(squeeze(press_s(1)).*(squeeze(eosben07_p_alpha(p1./1e4,p0./1e4,th_s(k),salt_s(k)).*1e4))./grav);
q2=q2+(squeeze(eosben07_p_alphap(p1./1e4,p0./1e4,th_s(k),salt_s(k))/grav)).*1e4*1e4;
end
dnm=-max(H(:)).*(squeeze(press_s(end)-press_s(1)))+q1-q2;
