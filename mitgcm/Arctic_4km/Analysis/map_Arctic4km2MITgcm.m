...
function Mfield = map_Arctic4km2MITgcm(Pfld,lonMIT,latMIT,PlonsGlob,PlatsGlob,Mlevs,Plevs,nanrepval) 
...
Pf_new = Plandval*ones(size(Plons,1),size(Plats,2),length(Mlevs)) ;
fprintf(1,' Interpolate PHC levs to MITgcm depths...') ;
for jj = 1:size(Plats,2)
   fprintf(1,'.') ; 
...
for kk = 1:length(Mlevs)   % Loop over MITgcm levels.
   ...
   %assign values to land pts based on closest sea points
    if(~isempty(lndmask) & ~isempty(seamask))
        tmpPf(lndmask) = griddata(Plons(seamask),Plats(seamask),tmpPf(seamask),Plons(lndmask),Plats(lndmask),'nearest');
    end
