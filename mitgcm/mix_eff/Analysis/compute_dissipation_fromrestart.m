function [epsilon epsilon_rho]=compute_dissipation_fromrestart(rho,u,v,w,u_rho,v_rho,w_rho,visc,delta_x,delta_y,delta_z,Nx,Ny,Nz)
...
dudy(:,2:Ny,:)=(u(2:end,2:end,:)-u(2:end,1:end-1,:))./(delta_y);
dudy(end+1,:,:)=dudy(1,:,:);                                                                            
dudy(:,end+1,:)=dudy(:,1,:);
dnm=visc.*dudy.^2;
clear dudy
...
% y-component
dvdx(2:Nx,:,:)=(v(2:end,2:end,:)-v(1:end-1,2:end,:))./(delta_x);
dvdx(end+1,:,:)=dvdx(1,:,:);                                                                            
dvdx(:,end+1,:)=dvdx(:,1,:);
dnm=visc.*dvdx.^2;
clear dvdx
...
% z-component
dwdx_u(2:Nx,:,:)=(w_rho(2:end,:,:)-w_rho(1:end-1,:,:))./(delta_x);
dwdx_u(Nx+1,:,:)=0.0;
...
dwdy_v(:,2:Ny,:)=(w_rho(:,2:end,:)-w_rho(:,1:end-1,:))./(delta_y);
dwdy_v(:,Ny+1,:)=0.0;
