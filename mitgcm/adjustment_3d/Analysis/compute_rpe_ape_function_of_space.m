...
%%   if u is constant; Mohammed's way, not sure if I can use it here
...
%                  +0.5*(vvel(i,j+1,k)-abs(vvel(i,j+1,k)))*(tmp(i,j+2,k)-tmp(i,j+1,k))/delta_y(i,j,k);
    ...
    flxBPEx(i,j,k,real_ind)=0.5*(uvel(i,j,k)+abs(uvel(i,j,k)))*(tmp(i+1,j+1,k+1)-tmp(i,j+1,k+1))/delta_x(i,j,k) ...
                  +0.5*(uvel(i+1,j,k)-abs(uvel(i+1,j,k)))*(tmp(i+2,j+1,k+1)-tmp(i+1,j+1,k+1))/delta_x(i,j,k);
    flxBPEy(i,j,k,real_ind)=0.5*(vvel(i,j,k)+abs(vvel(i,j,k)))*(tmp(i+1,j+1,k+1)-tmp(i+1,j,k+1))/delta_y(i,j,k) ...
                  +0.5*(vvel(i,j+1,k)-abs(vvel(i,j+1,k)))*(tmp(i+1,j+2,k+1)-tmp(i+1,j+1,k+1))/delta_y(i,j,k);
    ...
    if(abs(tmp2(i+1,j+1,k+1)-tmp(i+1,j+1,k+1))>1e-7)
      ...
      flxAPEz(i,j,k,real_ind)=0.5*(wvel(i,j,k)+abs(wvel(i,j,k)))*(tmp3(i+1,j+1,k+1)-tmp3(i+1,j+1,k))/delta_z(i,j,k) ...
                    +0.5*(wvel(i,j,k+1)-abs(wvel(i,j,k+1)))*(tmp3(i+1,j+1,k+2)-tmp3(i+1,j+1,k+1))/delta_z(i,j,k);

      flxPEz(i,j,k,real_ind)=0.5*(wvel(i,j,k)+abs(wvel(i,j,k)))*(tmp4(i+1,j+1,k+1)-tmp4(i+1,j+1,k))/delta_z(i,j,k) ...
                    +0.5*(wvel(i,j,k+1)-abs(wvel(i,j,k+1)))*(tmp4(i+1,j+1,k+2)-tmp4(i+1,j+1,k+1))/delta_z(i,j,k);
...
if 1
if time>0
  filename=[project_name '_ape_rpe_spatial_newfluxv4.mat'];
  save(['/export/grunchfs/unibjerknes/milicak/bckup/mitgcm/adjustment_3d/matfiles/' filename],'pe','rpe','ape','ape_new','dnm_ape','dnm_pe','dnm_rpe','flxBPEx','flxBPEy','flxBPEz','rhow','irr_mix','time_days','flxAPEz','flxPEz','wrhoz_zdz')
end
end
