...
for k=1:length(n)
  ...
  pcolorjw(y./1e3,-Z,dnm');shading flat;colorbar
  caxis([1025.4 1026.2])
  colorbar
  ...
  if(itr(k)==0)
    printname=['paperfigs/mitgcm_adjustment_3d_yzsection_spurious_mix_' [project_name] '_initial.eps']
  else
    printname=['paperfigs/mitgcm_adjustment_3d_yzsection_spurious_mix_' [project_name] '_' num2str(itr(k)) '.eps']
  end
  ...
  xlabel('y [km]')
  if(itr(k)==0)
    printname=['paperfigs/mitgcm_adjustment_3d_yzsection_spurious_mix_' [project_name] '_ape_initial.eps']
  else
    printname=['paperfigs/mitgcm_adjustment_3d_yzsection_spurious_mix_' [project_name] '_ape_' num2str(itr(k)) '.eps']
  end
  print(1,'-depsc2','-r300',printname);
  close
  ...
  if k==2
    ...
    ylabel('z [m]')
    xlabel('y [km]')
    printname=['paperfigs/mitgcm_adjustment_3d_yzsection_spurious_mix_' [project_name] '_rpe_' num2str(itr(k)) '.eps']
    print(1,'-depsc2','-r300',printname);
    close
  end
end
