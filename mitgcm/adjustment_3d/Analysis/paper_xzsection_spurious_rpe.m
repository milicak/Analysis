...
for k=1:2
  if k==2
    ...
    %pcolorjw(x*1e-3,-Z,abs(dnm2)');shading flat;colorbar
    imagesc(xc(:,1)*1e-3,-Z,abs(dnm2)');shading flat;colorbar
    axis xy
    caxis([0 .5])
    colorbar
    ...
    xlabel('x [km]')
%keyboard
    printname=['paperfigs/mitgcm_adjustment_3d_xzsection_spurious_mix_' [project_name] '_spurious_rpe_' num2str(n(1)) '_' num2str(n(2)) '.eps']
