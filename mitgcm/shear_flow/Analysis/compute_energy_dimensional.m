...
project_name=['Exp01.7']
alpha_T=3.03; %alpha 0.2 for Exp1.3 ; 2 for Exp1.1 ; 10 for Exp1.5 ; 6.06 for Exp1.4 ; 3.03 for Exp1.7
nondimensional=false
hdim='hvrt'

foldername=[root_name project_name '/']

delta_vort=0.02; %meter of the vorticity thickness
H=0.2; %meter
L=H*1;
W=H*0.5;
visc=1e-6;
...
rho0=1e3;
skip=1;
dumpfreq=0.5; %0.5; %0.025;   % %from data in the run folder
deltat=0.0005; %0.001;  %0.0005; %from data in the run folder
...
u_rho = u_rho./(2*u_charc);
v_rho = v_rho./(2*u_charc);
w_rho = w_rho./(2*u_charc);
end
...
save(['matfiles/' filename],'rpe','pe','ape','ke','ke_bouss','dissp','dissp_rho', 'wb','time_days','delta_rho','T_buoy','u_charc','Re','psi','delta_vort')
else
