...
switch title
    case 'thermobaricity_2D'
        ...
        %break
        save(['matfiles/mitgcm_init_ctrl_TS' expid '.mat'],'Tref','Sref','Trefold','Srefold','zr')
        %additional warming
        if 1
        ...
        % return
        % setup for Coarse resolution
        nx = 8; %512 meter resolution
        % 46 layers
        dzc = [5 5 5 5 5 5 5 10 10 10 10 10 10 10 10 10 10 10 20 20 25 25 25 50 50 50 ... 
               90 100 100 100 100 100 100 100 100 100 100 100 100 100 ...
               200 200 200 200 200 200];
        nz = length(dzc);
        zwc = [0 cumsum(dzc)];
        zrc = 0.5*(zwc(1:end-1)+zwc(2:end));
        clear dx
        dx(1,1:nx) = Lx/nx;
        ny = 1; 
        % bathymetry HAS TO BE NEGATIVE
        ...
        salt = zeros([nx,ny,nz]);
        temp = zeros([nx,ny,nz]);
        Snew = interp1(zr,Sref,zrc);
        Tnew = interp1(zr,Tref,zrc);
        Sref = Snew;
        Tref = Tnew;
        % Initial profile
        ...
        nzrand = 9; % first 9 layers 50 meter noise
        aa = 0.001*(1-rand(nx,ny,nzrand));
        randnoise(:,:,1:nzrand) = aa;
        if 0
          aa = zeros(nx,1);
          aa(2:end-1) = (0.001*(1-rand(nx-2,ny,1)));
          for k=1:nzrand
             randnoise(:,:,k) = aa*((nzrand-k)/(nzrand-1));
          end
        end
        temp = temp + randnoise;
...
%write delta z
delta_z=dzc';
fid=fopen(['dz.bin'],'w',ieee); fwrite(fid,delta_z,prec); fclose(fid);

%write Salt and Temp
fid=fopen(['S.init'],'w',ieee); fwrite(fid,salt,prec); fclose(fid);
fid=fopen(['T.init'],'w',ieee); fwrite(fid,temp,prec); fclose(fid);
