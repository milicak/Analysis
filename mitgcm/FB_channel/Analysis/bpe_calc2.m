function [BPE,FBPE,HPE,bpe,rhos,zs,vol_sorted] = bpe_calc2(theta_model,dz_model,area_model,h_model,h_topog,vol_topog,gravity)
...
%  dxdydz(:,:,k)=area_model.*squeeze(dz_model(:,:,1)); % Volumes of cells on model grid
%end
for k=1:size(dz_model,3)
  dxdydz(:,:,k)=area_model.*squeeze(dz_model(:,:,3)); % Volumes of cells on model grid
end
