function [BPE,PE,FBPE,HPE,bpe,rhos,zs,vol_sorted] = bpe_calc2(theta_model,salt_model,dz_model,area_model,h_model,h_topog,vol_topog,gravity)
...
for k=1:size(dz_model,1)
  dxdydz(k,:,:)=area_model.*squeeze(dz_model(k,:,:)); % Volumes of cells on model grid
end
fprintf('bpe_calc: ocean volume = %.14g m^3 (from model)\n',sum(dxdydz(:)))
%Mehmet
p_model=int_for_p2(theta_model,salt_model,dz_model,gravity);
rho_model=this_eos(theta_model,salt_model,p_model); rho_model=rho_model-(max(rho_model(:))+min(rho_model(:)))/2;
...
sigma_model=this_eos(theta_model,salt_model,2000e4);
...
rhos=this_eos(thetas,salts,ps); rhos=rhos-(max(rhos)+min(rhos))/2;
...
pep=gravity*(z_cov).*(rhos-mean_rho).*vols;
...
%Mehmet
area3d=repmat(area_model,[1 1 size(dz_model,1)]);
area3d=permute(area3d,[3 1 2]);
Pi=zeros(size(dz_model,1)+1,size(dz_model,2),size(dz_model,3));
Pi(2:end,:,:)=-nancumsum(dz_model);
zr=0.5*(Pi(1:end-1,:,:)+Pi(2:end,:,:));
dnm=gravity*rho_model.*dz_model.*zr.*area3d;
PE=nansum(dnm(:))/total_area; % Return J/m^2 (frame invariant PE)
...
function [rho,al0,lambda,p0] = this_eos(T,S,P)
...
function [P,rho] = int_for_p2(T,S,dz,gravity)

P=0*T+nancumsum(dz)*gravity*1025; % Initial guess fo pressure
rho=this_eos(T,S,P);
%Pio=[];
for it=1:5;
  dp=gravity*rho.*dz;
  %Pi=[0 nancumsum(dp)];
  Pi=zeros(size(dp,1)+1,size(dp,2),size(dp,3));
  Pi(2:end,:,:)=nancumsum(dp);
%  if isempty(Pio)
%    Pio=Pi;
%  else
%    stats(Pi-Pio)
...
rhou=this_eos(T(1:end-1),S(1:end-1),pi);
rhod=this_eos(T(2:end),S(2:end),pi);
ni=numel(rhod);
j=find(rhod<rhou);
