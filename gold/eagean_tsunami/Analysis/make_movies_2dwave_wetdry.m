...
iniland = squeeze(e1(:,:,1)).*(mask1-mask2);
e2 = ncread(fname,'e');
...
for k=1:60    
    ...
    switch colormaptype
        ...
        case char('gebcoland')
            [c,hc]=m_contourf(lon,lat,topo,C(14:end),'linecolor','none');
            colormap(gebco_land)
            hb=cbarfmi([0 C(end)],C(14:end),'horizontal','nonlinear');
            cdatamidlev([hc;hb],C,'nonlinear');
        case char('demcmap')
            m_pcolor(lon,lat,topo);shading flat 
            demcmap([0 2000])
        %end
    end
    hold on
    [ii jj]=find((wetland-iniland)>=0.01 & (wetland-iniland)<0.3);
    if(isempty(ii) == 0)
        for i=1:length(ii)
            m_plot(lon(ii(i),jj(i)),lat(ii(i),jj(i)),'o','MarkerEdgecolor','c','MarkerFacecolor','c')
        end
    end
    [ii jj]=find((wetland-iniland)>=0.3 & (wetland-iniland)<0.6);
    if(isempty(ii) == 0)
        for i=1:length(ii)
            m_plot(lon(ii(i),jj(i)),lat(ii(i),jj(i)),'o','MarkerEdgecolor','g','MarkerFacecolor','g')
        end
    end
    [ii jj]=find((wetland-iniland)>=0.6);
    if(isempty(ii) == 0)
        for i=1:length(ii)
            m_plot(lon(ii(i),jj(i)),lat(ii(i),jj(i)),'o','MarkerEdgecolor','r','MarkerFacecolor','r')
        end
    end
    %freezeColors
    m_grid
    printname=['gifs/Bodrum_ssh_2d_wetdry' no];                                           
