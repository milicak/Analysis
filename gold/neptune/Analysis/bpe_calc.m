function [BPE,FBPE,HPE,bpe,rhos,zs,vol_sorted] = bpe_calc(theta_model,salt_model,dz_model,area_model,h_model,h_topog,vol_topog,gravity)
...
elseif(eos=='jckmcd')
sigma_model=sw_dens_new(salt_model,theta_model,2000);
elseif(eos=='linear')
sigma_model=rho0-0.2*theta_model+0.8*salt_model;
end
...
ps=int_for_p(thetas,salts,dzs,gravity);
%ps=2000e4+0*zs; % uncomment to use sigma2 for BPE calculation
ps=ps*1e-4; % convert pascal to db
rhos=sw_dens_new(salts,thetas,ps); rhos=rhos-(max(rhos)+min(rhos))/2;
elseif(eos=='linear')
rhos=-0.2*thetas+0.8*salts; rhos=rhos-(max(rhos)+min(rhos))/2;
end
