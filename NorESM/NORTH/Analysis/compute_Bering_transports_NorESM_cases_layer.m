...
project_name='NAER1850CNOC_f19_g16_04'
yearsini=300; %2006;
yearsend=599; %2100;
...
NSS=[BS]; % total Bering Strait Section
for i=1:size(NSS,1)
...
for time=yearsini:yearsend
   for month=1:12
     ...
     trBS=transsect;
     TrBS(timeind)=nansum(trBS(:));
     HTrBS(timeind)=nansum(heattranssect(:))*1e-6;
     STrBS(timeind)=nansum(salttranssect(:))*1e-6;

     savename=['matfiles/' modelname '_' project_name '_' num2str(yearsini) '_' num2str(yearsend) '_Bering_transports'];
     save(savename,'TrBS','HTrBS','STrBS')
