...
for time=yearsini:yearsend
   for month=1:12
     ...
     trBS=transsect;
     TrCAA(timeind)=nansum(trBS(:));
     HTrCAA(timeind)=nansum(heattranssect(:))*1e-6;
     STrCAA(timeind)=nansum(salttranssect(:))*1e-6;

     savename=['matfiles/' modelname '_' project_name '_' num2str(yearsini) '_' num2str(yearsend) '_CAA_transports'];
     save(savename,'TrCAA','HTrCAA','STrCAA')
