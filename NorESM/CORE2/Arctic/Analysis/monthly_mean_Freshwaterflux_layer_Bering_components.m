...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/' expid '_heat_freshwater_layer_Bering_new_monthly_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
for year=fyear:lyear
  ...
  for month=1:12
    ...
    for k=1:size(bering,1)
      tmp3(k,:)=tmpu(bering(k,1),bering(k,2),:).*bering(k,3)+tmpv(bering(k,1),bering(k,2),:).*bering(k,4);
      tmp3(k,:)=tmpu(bering(k,1),bering(k,2),:).*dx(bering(k,1),bering(k,2)).*bering(k,3) ...
               +tmpv(bering(k,1),bering(k,2),:).*dy(bering(k,1),bering(k,2)).*bering(k,4);
      saltsectmp(k,:)=saln(bering(k,1),bering(k,2),:);
    end
