...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/' expid '_heat_freshwater_layer_Fram_new_monthly_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
%dlsec_varid=netcdf.defVar(ncid,'dlsec','float',[ni_dimid nj_dimid]);
%netcdf.putAtt(ncid,dlsec_varid,'long_name','Total transport');
%netcdf.putAtt(ncid,dlsec_varid,'units','m/s');
%netcdf.putAtt(ncid,dlsec_varid,'_FillValue',single(fill_value));
...
for year=fyear:lyear
  ...
  for month=1:12
    ...
    for k=1:size(fram,1)
      tmp3(k,:)=tmpu(fram(k,1),fram(k,2),:).*dx(fram(k,1),fram(k,2)).*fram(k,3) ...
               +tmpv(fram(k,1),fram(k,2),:).*dy(fram(k,1),fram(k,2)).*fram(k,4);
      saltsectmp(k,:)=saln(fram(k,1),fram(k,2),:);
    end
