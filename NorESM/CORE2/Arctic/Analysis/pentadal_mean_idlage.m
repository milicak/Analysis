% this subroutine computes pendatal mean idlage
clear all
...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/' expid '_idealage_pendatal_' num2str(fyear) '-' num2str(lyear2) '.nc'],'NC_CLOBBER');
...
idlage_varid=netcdf.defVar(ncid,'idlage','float',[ni_dimid nj_dimid nz_dimid time_dimid]);
...
netcdf.putAtt(ncid,idlage_varid,'coordinates','TLON TLAT');
netcdf.putAtt(ncid,idlage_varid,'cell_measures','area: tarea');
...
for year=fyear:pendatal+1:lyear
  ...
  for year_in=year:year+pendatal
    ...
    for month=1:12
      ...
      disp(sdate)
      tmp=ncgetvar([prefix sdate '.nc'],'idlagelvl');
      idlage=idlage+tmp(:,1:end-1,:);
      tmp=ncgetvar([prefix sdate '.nc'],'time');
      time=time+tmp;
    end
  end
  %idlage=idlage/(12*nn);
  idlage=idlage/(yeardays*nn);
  time=time/(12*nn);
  idlage(isnan(idlage))=fill_value;
  display('netcdf data is being written')
  netcdf.putVar(ncid,time_varid,n-1,1,single(time));
  netcdf.putVar(ncid,idlage_varid,[0 0 0 n-1],[nx ny depth 1],single(idlage));
end
