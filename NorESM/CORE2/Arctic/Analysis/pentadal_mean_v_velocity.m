% this subroutine computes pendatal mean v-velocity
clear all
...
%ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/' expid '_v_velocity_pendatal_' num2str(fyear) '-' num2str(lyear2) '.nc'],'NC_CLOBBER');
ncid=netcdf.create(['/fimm/work/milicak/RUNS/noresm/CORE2/Arctic/' expid '_v_velocity_pendatal_' num2str(fyear) '-' num2str(lyear2) '.nc'],'NC_CLOBBER');
...
for year=fyear:pendatal+1:lyear
  ...
  for year_in=year:year+pendatal
    ...
    for month=1:12
      ...
      vvel=vvel+tmp(:,1:end-1,:)*months2days(month);
      tmp=ncgetvar([prefix sdate '.nc'],'time');
      time=time+tmp;
    end
  end
  %vvel=vvel/(12*nn);
  vvel=vvel/(yeardays*nn);
  time=time/(12*nn);
