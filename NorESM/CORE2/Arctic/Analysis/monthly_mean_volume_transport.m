...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/' expid '_volume_transport_monthly_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
volume_tr_varid=netcdf.defVar(ncid,'volume_tr','float',[ni_dimid time_dimid]);
netcdf.putAtt(ncid,volume_tr_varid,'long_name','Northward heat flux');
netcdf.putAtt(ncid,volume_tr_varid,'units','kg/s');
netcdf.putAtt(ncid,volume_tr_varid,'_FillValue',single(fill_value));
...
for year=fyear:lyear
  for month=1:12
    ...
    disp(sdate)
    tmp=ncgetvar([prefix sdate '.nc'],'voltr');
    volume_tr=tmp(1,1)+tmp(2,1)+tmp(3,1)+tmp(10,1);
    vol_barents(n)=tmp(1,1);
    vol_bering(n)=tmp(2,1);
    vol_caa(n)=tmp(3,1);
    vol_fram(n)=tmp(10,1);
    volume_tr(isnan(volume_tr))=fill_value;
    tmp=ncgetvar([prefix sdate '.nc'],'time');
    time=tmp;
    netcdf.putVar(ncid,time_varid,n-1,1,single(time));
    netcdf.putVar(ncid,volume_tr_varid,[0 n-1],[nx 1],single(volume_tr));
  end
end
