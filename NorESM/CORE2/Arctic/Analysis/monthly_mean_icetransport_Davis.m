...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/' expid '_ice_transport_Davis_monthly_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
for year=fyear:lyear
  for month=1:12
    ...
    for k=1:size(davis,1)    
      dnm=tmp(davis(k,1)-1,davis(k,2),:);
      dnm(isnan(dnm))=0;
      dnm2=tmp4(davis(k,1),davis(k,2)-1,:);
      dnm2(isnan(dnm2))=0; 
      tmp2(k,:)=dnm.*davis(k,3)+dnm2.*davis(k,4);
    end
    tmp2=tmp2./rhoice;
