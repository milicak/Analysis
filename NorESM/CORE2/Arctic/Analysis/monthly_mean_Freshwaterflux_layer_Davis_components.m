...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/' expid '_heat_freshwater_layer_Davis_new_monthly_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
for year=fyear:lyear
  ...
  for month=1:12
    ...
    for k=1:size(davis,1)
      tmp3(k,:)=tmpu(davis(k,1),davis(k,2),:).*dx(davis(k,1),davis(k,2)).*davis(k,3) ...
               +tmpv(davis(k,1),davis(k,2),:).*dy(davis(k,1),davis(k,2)).*davis(k,4);
      saltsectmp(k,:)=saln(davis(k,1),davis(k,2),:);
    end
