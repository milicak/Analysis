...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/' expid '_ice_transport_Bering_monthly_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
for year=fyear:lyear
  for month=1:12
    ...
    for k=1:size(bering,1)    
      dnm=tmp(bering(k,1)-1,bering(k,2),:);
      dnm(isnan(dnm))=0;
      dnm2=tmp4(bering(k,1),bering(k,2)-1,:);
      dnm2(isnan(dnm2))=0; 
      tmp2(k,:)=dnm.*bering(k,3)+dnm2.*bering(k,4);
    end
    tmp2=tmp2./rhoice;
