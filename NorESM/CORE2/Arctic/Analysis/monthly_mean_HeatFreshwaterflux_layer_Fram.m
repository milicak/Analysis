...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/' expid '_heat_freshwater_layer_Fram_monthly_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
FWvflxtotal_varid=netcdf.defVar(ncid,'FWvflx_total','float',[ni_dimid time_dimid]);
netcdf.putAtt(ncid,FWvflxtotal_varid,'long_name','Total fresh water flux');
netcdf.putAtt(ncid,FWvflxtotal_varid,'units','kg s-1');
netcdf.putAtt(ncid,FWvflxtotal_varid,'_FillValue',single(fill_value));
...
for year=fyear:lyear
  for month=1:12
    ...
    tmp4(dp==0)=0;
    for k=1:size(fram,1)    
      tmp3(k,:)=tmp4(fram(k,1),fram(k,2),:).*fram(k,3)+tmp(fram(k,1),fram(k,2),:).*fram(k,4);
    end
    ...
    tmp4(dp==0)=0;
    for k=1:size(fram,1)    
      tmp2(k,:)=tmp4(fram(k,1),fram(k,2),:).*fram(k,3)+tmp(fram(k,1),fram(k,2),:).*fram(k,4);
    end
    ...
    FWvflxtotal(isnan(FWvflxtotal))=fill_value;
    ...
    netcdf.putVar(ncid,FWvflxtotal_varid,[0 n-1],[nx 1],single(FWvflxtotal));
