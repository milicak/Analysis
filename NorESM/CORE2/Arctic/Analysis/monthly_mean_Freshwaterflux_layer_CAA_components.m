...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/' expid '_heat_freshwater_layer_CAA_new_monthly_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
for year=fyear:lyear
  ...
  for month=1:12
    ...
    for k=1:size(caa,1)
      tmp3(k,:)=tmpu(caa(k,1),caa(k,2),:).*dx(caa(k,1),caa(k,2)).*caa(k,3) ...
               +tmpv(caa(k,1),caa(k,2),:).*dy(caa(k,1),caa(k,2)).*caa(k,4);
      saltsectmp(k,:)=saln(caa(k,1),caa(k,2),:);
    end
