clear all
sec_depth=400; %section depth 5 or 400
sec_depth=400
...
titles = [{'CERFACS'} {'gold'} {'MRI-F'} {'NOC'} ...
         ...
         {'CNRM'} {'Kiel-ORCA05'} {'NCAR'} {'AWI-FESOM'} {'FSU-HYCOMv2'}];
...
if(i==13)
  ...
  dd=[dd1;dd2]; 
  %dd(isnan(dd)==1)=0;
  salt_model_woa=salt_model_woa+dd;
  dd=reshape(S*t_a,nx_b,ny_b);
  dd1=dd(181:end,:);
  dd2=dd(1:180,:);
  dd=[dd1;dd2]; 
  %dd(isnan(dd)==1)=0;
  temp_model_woa=temp_model_woa+dd;
else
  salt_model_woa=salt_model_woa+reshape(S*s_a,nx_b,ny_b);
  temp_model_woa=temp_model_woa+reshape(S*t_a,nx_b,ny_b);
  %dnm=reshape(S*s_a,nx_b,ny_b);
  %dnm(isnan(dnm)==1)=0;
  %salt_model_woa=salt_model_woa+dnm;
  %dnm=reshape(S*t_a,nx_b,ny_b);
  %dnm(isnan(dnm)==1)=0;
  %temp_model_woa=temp_model_woa+dnm;
end
i
end

%temp_model_woa=temp_model_woa./length(files);
%salt_model_woa=salt_model_woa./length(files);

temp_model_woa=temp_model_woa./i;
salt_model_woa=salt_model_woa./i;
...
m_pcolor(lon,lat,salt_model_woa');shading interp;needJet2;caxis([34.2 35.2])
if(sec_depth==5)
caxis([25 35])
...
m_grid('xtick',[-120 -60 0 60 120])
     ...
     set(gcf,'color','w');
printname=['paperfigs2/mmm_salt_horizontal_section_' num2str(sec_depth)];
...
figure(1)
m_proj('stereographic','lat',90,'long',0,'radius',25);
m_pcolor(lon,lat,temp_model_woa');shading interp;needJet2;caxis([-2 5])
if(sec_depth==5)
caxis([-1 8])
...
m_grid('xtick',[-120 -60 0 60 120])
     ...
     set(gca,'fontsize',fontsize)
     set(gcf,'color','w');
printname=['paperfigs2/mmm_temp_horizontal_section_' num2str(sec_depth)];
...
m_grid('xtick',[-120 -60 0 60 120])
     ...
     set(gca,'fontsize',fontsize)
     set(gcf,'color','w');
printname=['paperfigs2/mmm_salt_bias_horizontal_section_' num2str(sec_depth)];
print(1,'-depsc2','-r150',printname)
export_fig(1,printname,'-eps','-r150');
close all
...
m_grid('xtick',[-120 -60 0 60 120])
     ...
     set(gca,'fontsize',fontsize)
     set(gcf,'color','w');
printname=['paperfigs2/mmm_temp_bias_horizontal_section_' num2str(sec_depth)];
print(1,'-depsc2','-r150',printname)
export_fig(1,printname,'-eps','-r150');
close all
