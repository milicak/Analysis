...
filename_s='/fimm/work/milicak/RUNS/noresm/CORE2/Arctic/DATA/NorESM/NOIIA_T62_tn11_sr10m60d_01_salinity_pendatal_1-300.nc';
filename_t='/bcmhsm/milicak/RUNS/noresm/CORE2/Arctic/DATA/NorESM/NOIIA_T62_tn11_sr10m60d_01_temperature_pendatal_1-300.nc';
filename_u='/fimm/work/milicak/RUNS/noresm/CORE2/Arctic/DATA/NorESM/NOIIA_T62_tn11_sr10m60d_01_u_velocity_pendatal_1-300.nc';
filename_v='/fimm/work/milicak/RUNS/noresm/CORE2/Arctic/DATA/NorESM/NOIIA_T62_tn11_sr10m60d_01_v_velocity_pendatal_1-300.nc';
uvel=ncgetvar(filename_u,'uvel');
vvel=ncgetvar(filename_v,'vvel');
salt=ncgetvar(filename_s,'saln');
temp=ncgetvar(filename_t,'temp'); 
lon=ncgetvar(filename_s,'TLON');
lat=ncgetvar(filename_s,'TLAT');
zt=ncgetvar(filename_s,'depth'); %convert cm to meter
...
set(gcf,'color','w');
printname=['paperfigs2/noresm_micom_uv_horizontal_section_' num2str(sec_depth)];
...
m_grid('xtick',[-120 -60 0 60 120])
     ...
     set(gca,'fontsize',fontsize)
     set(gcf,'color','w');
%printname=['paperfigs/noresm_micom_temp_horizontal_section_' num2str(sec_depth)];
printname=['paperfigs2/noresm_micom_temp_horizontal_section_' num2str(sec_depth)];
print(1,'-depsc2','-r150',printname)
...
m_grid('xtick',[-120 -60 0 60 120])
     ...
     set(gcf,'color','w');
%xlabel('Lon'); ylabel('Lat');
%printname=['paperfigs/noresm_micom_salt_horizontal_section_' num2str(sec_depth)];
printname=['paperfigs2/noresm_micom_salt_horizontal_section_' num2str(sec_depth)];
print(1,'-depsc2','-r150',printname)
export_fig(1,printname,'-eps','-r150');
close all
...
m_coast('patch',[.7 .7 .7],'edgecolor','k');
m_grid
printname=['paperfigs/noresm_micom_uv_horizontal_section_' num2str(sec_depth)];
print(1,'-depsc2','-r150',printname)
close all
