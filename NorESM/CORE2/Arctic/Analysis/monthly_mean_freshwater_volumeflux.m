...
fwflx_varid=netcdf.defVar(ncid,'fwflx','float',[ni_dimid nj_dimid time_dimid]);
netcdf.putAtt(ncid,fwflx_varid,'long_name','Fresh water flux');
netcdf.putAtt(ncid,fwflx_varid,'units','kg m-2 s-1');
netcdf.putAtt(ncid,fwflx_varid,'_FillValue',single(fill_value));
netcdf.putAtt(ncid,fwflx_varid,'coordinates','TLON TLAT');
netcdf.putAtt(ncid,fwflx_varid,'cell_measures','area: tarea');
...
for year=fyear:lyear
  for month=1:12
    ...
    netcdf.putVar(ncid,fwflx_varid,[0 0 n-1],[nx ny 1],single(fwflx));
  end
end
