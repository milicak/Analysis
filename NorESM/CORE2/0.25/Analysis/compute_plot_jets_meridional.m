...
datesep='-';
%grid_file='/work/shared/noresm/inputdata/ocn/micom/tnx0.25v1/20130930/grid.nc';
...
disp(sdate)
%surface
v1(:,:,:,year-fyear+1)=nc_varget([prefix sdate '.nc'],'vvellvl',[0 0 0 0],[-1 1 -1 -1]);
%full column
%v1(:,:,:,year-fyear+1)=nc_varget([prefix sdate '.nc'],'vvellvl',[0 0 0 0],[-1 -1 -1 -1]);
end
vmean=squeeze(nanmean(v1,4));
...
%surface
vjet_f = filter2(b,vmean)./sum(b(:));
%full column
%for k=1:size(vmean,1)
%  vjet_f(k,:,:) = filter2(b,squeeze(vmean(k,:,:)))./sum(b(:));
%end
...
for year=fyear:lyear
  ...
  for month=1:12
    ...
    disp(sdate)
    tmp=tmp+nc_varget([prefix sdate '.nc'],'vvellvl',[0 0 0 0],[-1 1 -1 -1]).*months2days(month);
  end
  tmp=tmp./yeardays;
  u1(:,:,year-fyear+1)=tmp;
end
