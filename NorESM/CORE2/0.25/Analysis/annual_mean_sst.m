% this subroutine computes annual mean sst
clear all
...
if m2y==1
  ...
  grid_file='/fimm/work/milicak/RUNS/noresm/0.25/grid.nc';
end
...
% Create netcdf file.
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/0.25/' expid '_sst_annual_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
sst_varid=netcdf.defVar(ncid,'sst','float',[ni_dimid nj_dimid time_dimid]);
netcdf.putAtt(ncid,sst_varid,'long_name','Sea surface temperature');
netcdf.putAtt(ncid,sst_varid,'units','degC');
netcdf.putAtt(ncid,sst_varid,'_FillValue',single(fill_value));
netcdf.putAtt(ncid,sst_varid,'coordinates','TLON TLAT');
netcdf.putAtt(ncid,sst_varid,'cell_measures','area: tarea');
...
for year=fyear:lyear
  ...
  if m2y==1
    for month=1:12
      ...
      disp(sdate)
      tmp=ncgetvar([prefix sdate '.nc'],'sst');
      sst=sst+tmp(:,1:end-1).*months2days(month);
  ...
  sst(isnan(sst))=fill_value;
  netcdf.putVar(ncid,time_varid,n-1,1,single(time));
  netcdf.putVar(ncid,sst_varid,[0 0 n-1],[nx ny 1],single(sst));
end
