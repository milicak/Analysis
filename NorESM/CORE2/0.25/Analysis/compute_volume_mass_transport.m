...
for year=fyear:lyear
  if m2y==1
    tmp1=zeros(size(lat,1),70,3);
    tmp2=zeros(size(lat,1),70,3);
    for month=1:12
      sdate=sprintf('%4.4d%c%2.2d',year,datesep,month);
      disp(sdate)
      tmp=ncgetvar([prefix sdate '.nc'],'mmflxd');
      tmp1=tmp1+tmp.*months2days(month);
      tmp=ncgetvar([prefix sdate '.nc'],'mmftdd');
      tmp2=tmp2+tmp.*months2days(month);
    end
    n=n+1;
    mmflxd(n,:,:,:)=tmp1./(yeardays);
    mmftdd(n,:,:,:)=tmp2./(yeardays);
  else
     n=n+1;
...
save(['matfiles/' expid '_annual_volume_flux_' num2str(fyear) '_' num2str(lyear)],'mmflxd','mmftdd','lat');
