% this subroutine computes monthly mean net heat flux which is hflx
clear all
...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/SSH/' expid '_net_heatflux_monthly_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
Qnet_varid=netcdf.defVar(ncid,'net_heatflux','float',[ni_dimid nj_dimid time_dimid]);
netcdf.putAtt(ncid,Qnet_varid,'long_name','Net surface heat flux received by ocean');
...
for year=fyear:lyear
  for month=1:12
    ...
    disp(sdate)
    tmp=ncgetvar([prefix sdate '.nc'],'hflx');
    Qnet=tmp(:,1:end-1,:);
