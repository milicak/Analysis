% this subroutine computes annual mean bottompress
clear all
...
% Create netcdf file.
ncid=netcdf.create([expid '_bottompress_annual_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
bottompress_varid=netcdf.defVar(ncid,'bottompress','float',[ni_dimid nj_dimid time_dimid]);
netcdf.putAtt(ncid,bottompress_varid,'long_name','Bottom Pressure');
netcdf.putAtt(ncid,bottompress_varid,'units','Pa');
netcdf.putAtt(ncid,bottompress_varid,'_FillValue',single(fill_value));
netcdf.putAtt(ncid,bottompress_varid,'coordinates','TLON TLAT');
netcdf.putAtt(ncid,bottompress_varid,'cell_measures','area: tarea');
...
for year=fyear:lyear
  ...
  for month=1:12
    ...
    tmp=tmp(:,1:end-1,:);
    bottompress=bottompress+squeeze(sum(tmp,3));
    tmp=ncgetvar([prefix sdate '.nc'],'time');
    time=time+tmp;
  end
  bottompress=bottompress/12;
  time=time/12;
  bottompress(isnan(bottompress))=fill_value;
  netcdf.putVar(ncid,time_varid,n-1,1,single(time));
  netcdf.putVar(ncid,bottompress_varid,[0 0 n-1],[nx ny 1],single(bottompress));
end
