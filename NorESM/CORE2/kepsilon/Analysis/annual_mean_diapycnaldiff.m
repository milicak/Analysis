% this subroutine computes pendatal mean diapycnal diffusivity
clear all

%expid='NOIIA_T62_tn11_sr10m60d_01';
%userid='matsbn';
expid='NOIIA_T62_tn11_004'
userid='milicak';
datesep='-';
...
lyear=100;
lyear2=100;
pendatal=0; % averaged years
fill_value=-1e33;
           % Jan Feb Mar Apr May June July Aug Sep Oct Nov Dec
months2days=[31  28  31  30  31   30   31  31   30 31   30 31];
yeardays=sum(months2days);

prefix=['/hexagon/work/' userid '/archive/' expid '/ocn/hist/' expid '.micom.hm.'];
...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/kepsilon/' expid '_diapycnaldiff_annual_' num2str(fyear) '-' num2str(lyear2) '.nc'],'NC_CLOBBER');
...
difdia_varid=netcdf.defVar(ncid,'difdia','float',[ni_dimid nj_dimid nz_dimid time_dimid]);
netcdf.putAtt(ncid,difdia_varid,'long_name','Diapycnal diffusivity');
netcdf.putAtt(ncid,difdia_varid,'units','log10(m2 s-1)');
netcdf.putAtt(ncid,difdia_varid,'_FillValue',single(fill_value));
netcdf.putAtt(ncid,difdia_varid,'coordinates','TLON TLAT');
netcdf.putAtt(ncid,difdia_varid,'cell_measures','area: tarea');
...
for year=fyear:pendatal+1:lyear
  ...
  for year_in=year:year+pendatal
    ...
    for month=1:12
      ...
      disp(sdate)
      tmp=ncgetvar([prefix sdate '.nc'],'difdialvl');
      difdia=difdia+tmp(:,1:end-1,:).*months2days(month);
      tmp=ncgetvar([prefix sdate '.nc'],'time');
      time=time+tmp;
    end
  end
  difdia=difdia/(nn*yeardays);
  time=time/(12*nn);
  difdia(isnan(difdia))=fill_value;
  display('netcdf data is being written')
  netcdf.putVar(ncid,time_varid,n-1,1,single(time));
  netcdf.putVar(ncid,difdia_varid,[0 0 0 n-1],[nx ny depth 1],single(difdia));
end
