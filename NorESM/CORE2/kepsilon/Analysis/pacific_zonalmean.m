...
if(i<=296 & i>=120 & j >=35 & j<=156 & dnm(i,j)==1) 
dnm(i,j)=4;    
end
end;end
for i=1:nx;for j=1:ny
if(i<=300 & i>=260 & j >=140 & j<=160 & dnm(i,j)==4)
dnm(i,j)=1;
end
end;end
mask_woa09=dnm;
...
% Create 3D masks for pacific
mask_3d_woa09=reshape(reshape(mask_woa09,[],1)*ones(1,nz_b),nx_b,ny_b,nz_b);
mask_3d_woa09(find(isnan(t_woa09)|mask_3d_woa09~=4))=0; 
mask_3d_woa09(find(mask_3d_woa09==4))=1;
mask_3d_dst=reshape(reshape(mask_woa09,[],1)*ones(1,nz_a),nx_b,ny_b,nz_a);
mask_3d_dst(find(mask_3d_dst~=4))=0;
mask_3d_dst(find(mask_3d_dst==4))=1;
...
% Create netcdf file.
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/kepsilon/' expid '_pacific_zonalmean_timemean_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
