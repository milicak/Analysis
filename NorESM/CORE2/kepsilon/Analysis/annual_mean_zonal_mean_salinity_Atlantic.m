...
lyear=100;
fill_value=-1e33;
map_file='/home/fimm/bjerknes/milicak/Analysis/NorESM/climatology/Analysis/map_tnx1v1_to_woa09_aave_20120501.nc';
mask_file='/home/fimm/bjerknes/milicak/Analysis/NorESM/climatology/Analysis/atlantic_mask.dat';
...
fclose(fid);
world_ocean_mask_a(find(world_ocean_mask_a~=2))=0;
world_ocean_mask_a=world_ocean_mask_a(:,1:end-1);

% Create netcdf file.
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/kepsilon/' expid '_zonal_mean_salinity_annual_Atlantic_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
szm_varid=netcdf.defVar(ncid,'salt','float',[lat_dimid depth_dimid time_dimid]);
netcdf.putAtt(ncid,szm_varid,'long_name','salinity');
netcdf.putAtt(ncid,szm_varid,'units','g kg-1');
...
% Retrieve zonal salinity and write to netcdf variables
...
for year=fyear:lyear
  n=n+1;
  % Create annual mean
  salnlvl=zeros(nx_a,ny_a,nz_a);
  time=0;
  for month=1:12
    sdate=sprintf('%4.4d%c%2.2d',year,datesep,month);
    disp(sdate)
    tmp=ncgetvar([prefix sdate '.nc'],'salnlvl');
    salnlvl=salnlvl+tmp(:,1:end-1,:);
