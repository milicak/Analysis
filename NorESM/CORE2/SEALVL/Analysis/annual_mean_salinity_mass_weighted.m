...
ncid=netcdf.create(['/bcmhsm/milicak/RUNS/noresm/CORE2/SSH/' expid '_salinity_mass_weighted_annual_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
for year=fyear:lyear
  ...
  for month=1:12
    ...
    saln=saln+tmp(:,1:end-1,:).*tmp2(:,1:end-1,:).*months2days(month);
    tmp=ncgetvar([prefix sdate '.nc'],'time');
    time=time+tmp;
  end
  saln=(saln./dp)./yeardays;
  dp=dp./yeardays;
  time=time/12;
