...
lyear=25;
fill_value=-1e33;
%map_file='/home/nersc/matsbn/matlab/regrid/maps/map_tnx1v1_to_woa09_aave_20120501.nc';
...
% Create netcdf file.
ncid=netcdf.create([expid '_zonal_mean_temperature_annual_' num2str(fyear) '-' num2str(lyear) '.nc'],'NC_CLOBBER');
...
tzm_varid=netcdf.defVar(ncid,'theta','float',[lat_dimid depth_dimid time_dimid]);
netcdf.putAtt(ncid,tzm_varid,'long_name','potential temperature');
netcdf.putAtt(ncid,tzm_varid,'units','degC');
netcdf.putAtt(ncid,tzm_varid,'_FillValue',single(fill_value));
...
% Retrieve zonal temperature and write to netcdf variables
...
for year=fyear:lyear
  n=n+1;
  % Create annual mean
  templvl=zeros(nx_a,ny_a,nz_a);
  time=0;
  for month=1:12
    sdate=sprintf('%4.4d%c%2.2d',year,datesep,month);
    disp(sdate)
    tmp=ncgetvar([prefix sdate '.nc'],'templvl');
%    templvl=templvl+tmp(:,1:end-1,:);
    templvl=templvl+tmp;
  ...
  tzm=squeeze(nansum(t_b)./sum(weight_b));
  tzm(isnan(tzm))=fill_value;
  netcdf.putVar(ncid,time_varid,n-1,1,time);
  netcdf.putVar(ncid,tzm_varid,[0 0 n-1],[ny_b nz_a 1],single(tzm));
end
