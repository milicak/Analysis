...
SEG_cri=37.5; %critical salinity for the RED SEA overflow
...
% compute Atlantic Water, EGC, IFS, DS overflow temperature, salinity and transport    
     ...
     for i=1:size(NSS,1)
       ...
       transsect(:,i1)=(utr(:,NSS(i,2),NSS(i,1)+1).*NSS(i,3)+vtr(:,NSS(i,2),NSS(i,1)+1).*NSS(i,4))*1e-9;
       % for NAER1850CNOC_f19_g16_04-06 NRCP85AERCN_f19_g16_* type of models since heattrans is in Watts only
       heattranssect(:,i1)=(uhflx(:,NSS(i,2),NSS(i,1)+1).*NSS(i,3)+vhflx(:,NSS(i,2),NSS(i,1)+1).*NSS(i,4));
       % for NAER1850CNOC_f19_g16_03 type of models since heattrans is in Watts/m2 
       %heattranssect(:,i1)=(uhflx(:,NSS(i,2),NSS(i,1)+1).*NSS(i,3)+vhflx(:,NSS(i,2),NSS(i,1)+1).*NSS(i,4)).*areasect(:,i)/(Cp*rho0);
       salttranssect(:,i1)=(usflx(:,NSS(i,2),NSS(i,1)+1).*NSS(i,3)+vsflx(:,NSS(i,2),NSS(i,1)+1).*NSS(i,4));
       i1=i1+1;
     end
...
% Overflow water ==> the rest 
     ...
     if(abs(nansum(trEG(:))+nansum(trOW(:)) - nansum(transsect(:)))>1e-10)
