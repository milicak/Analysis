...
%
function  [image_data,yy,xx] = mygrid_sand(region)

%DatabasesDir = '/usr/local/bathy/';
DatabasesDir = '/home/mil021/Analysis/mitgcm/faroe/Analysis';

%DBfile = 'topo_6.2.img';
%DBfile = 'topo_8.2.img';
DBfile = 'topo_15.1.img'; % 1 minute resolution
...
%db_loc         = [-72.006 72.006 0.0 360-db_res]; % 2 minute resolution
db_loc         = [-80.738 80.738 0.0 360-db_res]; % 1 minute resolution
%db_size        = [6336 10800]; %2 minute resolution
db_size        = [17280 21600]; %1 minute resolution
...
for i = 1:length(wlon);
        ...
        % Open the data file
        fid = fopen([DatabasesDir '/' DBfile], 'r','b');
        if (fid < 0)
				[DatabasesDir '/' DBfile]
                errordlg(['Could not open database: ' DatabasesDir '/' DBfile],'Error');
        end
...
ielon
if(length(ielon)==2)
vlon=db_res*((iwlon(1)+1:ielon(1)+ielon(2)+2)-0.5);
else
vlon=db_res*((iwlon(1)+1:ielon(1)+1)-0.5);
end
