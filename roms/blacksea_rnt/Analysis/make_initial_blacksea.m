function make_initial_blacksea(fn,gn,ini,wdr,T1)
...
% copy clm time to init time
  ...
  %ocean_time=datenum('01-Jan-2012');
...
for i=1:length(vars2d)
    ...
    eval([vars2d{i},'=squeeze(',vars2d{i},'(:,:,1));']); %Mehmet squeeze eta for the first timestep
...
%    eval([vars3d{i},'=netcdf.getVar(nc_clm,tempclmid);']);%get data
     %%%% Mehmet %%%%
     eval([vars3d{i},'=nc_varget(''',fn,'''',',''',vars3d{i},''',[0 0 0 0],[1 -1 -1 -1]);']);
     eval([vars3d{i},'=permute(',vars3d{i},',[3 2 1]);']);
     %%%%%%%%%%%
...
%Mehmet
morvars={'theta_s','theta_b','Tcline','Cs_r','Cs_w','sc_w','sc_r','hc','ocean_time'};
...
netcdf.putVar(nc_init,tempid,gn.vtra);
tempid = netcdf.inqVarID(nc_init,'Vstretching');
netcdf.putVar(nc_init,tempid,gn.vstr);
clear tempid
