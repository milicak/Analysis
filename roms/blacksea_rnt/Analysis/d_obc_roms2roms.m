...
% svn $Id: d_obc_roms2roms.m 754 2015-01-07 23:23:40Z arango $
%=========================================================================%
...
NWAdata = 'blacksea-clim_all.nc';
NWAgrid = 'blacksea-grid.nc';

GRDname = 'blacksea-grid.nc';
BRYname = 'blacksea-bry.nc';

%StrDay = datenum('01-Jan-2012');        % starting day to process
%EndDay = datenum('31-Dec-2012');        % ending   day to process
StrDay = datenum(2012,01,02,12,0,0)
EndDay = datenum(2012,12,27,12,0,0);        % ending   day to process
...
S.Vtransform  = 1;          % vertical transfomation equation
...
S.hc          = 10.0;   % S-coordinate stretching width
...
time = nc_read(NWAdata,'salt_time');
epoch = datenum('01-Jan-0000');

%  Set reference time such that boundary conditions time are in
%  "seconds since 2000-01-01 00:00:00".

ref_time = (datenum('01-Jan-2012')-datenum('01-Jan-0000'))*86400;
...
% mehmet
StrRec = find(time == (StrDay-epoch));
EndRec = find(time == (EndDay-epoch));
...
for Rec = StrRec:EndRec,
  ...
  mydate = datestr(time(Rec)); % Mehmet my time is in days not units
...
%  Interpolate boundary conditions.
 ...
 display('the code breaks here, basically just overwrite values in the boundary file. Use make_bndry.m')		    
