...
for i=1:4
  filename=[matfiles char(projects(i)) '_mld_time_' num2str(fyear) '_' num2str(lyear) '.mat'];
  load(filename)
  figure(1)
  hold on
  plot(1:250,my_nanfilter(MLD_Lab,5,'box'),'color',[color(i,1) color(i,2) color(i,3)],'linewidth',2);
end
  xlabel('Time [years]')
  ylabel('Labrador Sea MLD depth [m]')
  legend('Cold-ctrl','Exp1','Exp2','Exp3')
printname = ['paperfigs/cold_exps_mldMarchtime.eps']
...
printname = ['paperfigs/warm_exps_mldMarchtime.eps']
...
printname = ['paperfigs/cold_warm_ctrls_mldMarchtime.eps']
